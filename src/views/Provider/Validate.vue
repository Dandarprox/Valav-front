    ...
    <div class="content">
      ...
      <div class="btn__container--vertical validation__actions">
        ...
        <transition name="trfade">
          <div 
            class="validation hlp__flex-column-center" 
            v-show="currentAction == 'register' && !actionsActive"
          >
            <div
              @click="closeActions" 
              class="validation__cancel">x</div>
            <span>Introduzca el valor en compras</span>
            <input 
              v-model="registerValue"
              type="number" class="validation__input">

            <div @click="registerConfirm" class="btn btn--solid">Aceptar</div>
          </div>
        </transition>
      ...
      <transition name="trfade">
        <div 
          class="hlp__pos-rel"
          v-show="currentAction == 'redeem' && !actionsActive"
        >
          <div @click="closeActions" class="validation__cancel">x</div>
          <Catalog
            @select="redeemConfirm"
      ...
      >
        <div class="btn__container--vertical">
          <div @click="hideModal" class="btn btn--solid">Aceptar</div>
...
import { Modal, product } from '../../types/general';
...
export default class ProviderValidate extends titleMixin {
  actionsActive = true;
  currentAction = '';
  registerValue = 0;
  modalTitle = '';
  ...
  private psMounted() {
    this.setLayoutTitle("Verificación");
  ...
  registerConfirm() {
    ...
    this.modalContent = `Has introducido compras por un valor de $${this.registerValue}`;
  }
  ...
  redeem() {
    this.actionsActive = false;
    this.currentAction = 'redeem';
  }

  redeemConfirm(product: product) {
    (this.$refs.modal as Modal).show();
    this.modalTitle = '¿Confirmar?';
    this.modalContent = `Has seleccionado ${product.title}, por ${product.value}pts.`;
  }
